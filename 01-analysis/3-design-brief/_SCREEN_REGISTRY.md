...
flowchart TD
    ...
    PUSH(["Push: подтверждение / напоминание / отмена студией"]) --> SCR009
```
...
| Push-уведомление (подтверждение / напоминание / отмена студией) | Тап по уведомлению | SCR-009 конкретной брони (резерв — SCR-008, см. раздел ниже) |
...
### Навигация из push-уведомлений (решено)

Вопрос закрыт по итогам уточнения с продуктом/разработкой:

- Бэкенд **всегда** передаёт в push ID конкретной брони, поэтому тап по
  любому push-уведомлению, привязанному к брони, ведёт **сразу на SCR-009**
  этой конкретной брони — независимо от того, сколько у пользователя
  активных броней.
- Это правило **одинаково для всех типов push**: подтверждение записи,
  напоминание за 2–3 часа, отмена слота студией.
- **Резервный сценарий** (на случай, если в будущем ID окажется недоступен —
  например, устаревшая версия приложения не смогла его обработать, либо
  бронь к моменту тапа уже не существует) — переход на SCR-008 (список «Мои
  брони»), чтобы пользователь не упирался в тупик или ошибку.

```mermaid
flowchart LR
    PUSH(["Push-уведомление (любой тип, привязанное к брони)"]) -->|ID брони известен — всегда| SCR009["SCR-009 Детали брони"]
    PUSH -.->|резервный сценарий: ID недоступен| SCR008["SCR-008 Мои брони"]
```

| Тип push | Куда ведёт тап |
|---|---|
| Подтверждение записи | SCR-009 (эта бронь) |
| Напоминание за 2–3 часа | SCR-009 (эта бронь) |
| Отмена слота студией | SCR-009 (эта бронь) |
| Любой тип, если ID брони недоступен (резерв) | SCR-008 (список) |
